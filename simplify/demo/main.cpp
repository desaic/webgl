...
std::vector<std::array<unsigned , 3> > TriangulatePolygon(const std::vector<std::array<float,3>> & v)
{

	//project to 2d
	Vec3f vertCenter(0, 0, 0);
	for (size_t vi = 0; vi < v.size() - 2; vi++)
	{
		vertCenter += Vec3f(v[vi][0], v[vi][1], v[vi][2]);
	}
	vertCenter = (1.0f / v.size()) * vertCenter;
	Vec3f signedArea(0, 0, 0);
	for (size_t vi = 0; vi < v.size(); vi++)
	{
		size_t vj = (vi + 1) % v.size();
		Vec3f v0(v[vi][0], v[vi][1], v[vi][2]);
		Vec3f v1(v[vj][0], v[vj][1], v[vj][2]);

		Vec3f e1 = v0 - vertCenter;
		Vec3f e2 = v1 - vertCenter;
		signedArea += e1.cross(e2);
	}
	Vec3f n = signedArea.normalizedCopy();
	Vec3f xAxis(1, 0, 0);
	if (std::abs(n[0]) > 0.9)
	{
		xAxis = Vec3f(0, 1, 0);
	...
	yAxis.normalize();
	xAxis = yAxis.cross(n);
	Vec3f v0(v[0][0], v[0][1], v[0][2]);
	std::vector<CDT::V2d<double> > verts(v.size());

	for (size_t vi = 0; vi < v.size(); vi++)
	{
		Vec3f vert(v[vi][0], v[vi][1], v[vi][2]);
		vert = vert - v0;
		verts[vi].x = vert.dot(xAxis);
		verts[vi].y = vert.dot(yAxis);
	}

	CDT::Triangulation<double> cdt;
	cdt.insertVertices(verts);
	std::vector<CDT::Edge> edges(verts.size(), CDT::Edge(0,0));
	for (size_t i = 0; i < edges.size(); i++)
	{
		edges[i] = CDT::Edge(unsigned(i), unsigned((i + 1) % edges.size()));
	}
	cdt.insertEdges(edges);
	cdt.eraseOuterTrianglesAndHoles();
	std::vector<CDT::Triangle> trigs = cdt.triangles;
	std::vector<std::array<unsigned, 3> > out(trigs.size());
	for (size_t i = 0; i < trigs.size(); i++)
	{
		//std::cout << trigs[i].vertices[0] << " " << trigs[i].vertices[1] << " " << trigs[i].vertices[2] <<"\n";
		for (unsigned j = 0; j < 3; j++)
		{
			out[i][j] = trigs[i].vertices[j];
		}
	}
	return out;
}
...
Mesh parseObj(const char* path, double& reindex)
{
	...
	for (unsigned int i = 0; i < obj->face_count; ++i)
	{
		...
		else
		{
			...
			std::vector<std::array<unsigned, 3> > trigs;
			trigs = TriangulatePolygon(points);
			for (size_t ti = 0; ti < trigs.size(); ti++)
			{
...
#include <string>
#include <fstream>
void SaveObj(const std::string& file, const Mesh& m)
{
  ...
  for (size_t i = 0; i < m.indices.size(); i+=3)
	{
	  out<<"f " << (m.indices[i] + 1) << " " << (m.indices[i + 1] + 1) << " " << (m.indices[i + 2] + 1) << "\n";
	}
	out.close();
}
...
#include<iostream>
#include <filesystem>
void TestSimplify(const std::string & inFile, const std::string outFile,
	float target_error, float target_count_frac)
{
  double reindexTime = 0.0;
	double tload = timestamp();
  std::cout << "start load obj time stamp " << tload << "\n";
	Mesh mesh = parseObj(inFile.c_str(), reindexTime);
	size_t target_index_count = std::max(1ull, size_t(mesh.indices.size() * target_count_frac));
	//size_t(mesh.indices.size() * 0.1);
	float result_error = 0;
	double t0 = timestamp();
	std::cout << "end load obj " << t0 << "\n";
	std::cout << "load obj time " << (t0 - tload) << "\n";
  Mesh lod;
	lod.indices.resize(mesh.indices.size()); // note: simplify needs space for index_count elements in the destination array, not target_index_count
  lod.indices.resize(meshopt_simplify(&lod.indices[0], &mesh.indices[0], mesh.indices.size(), &mesh.vertices[0].px, mesh.vertices.size(), sizeof(Vertex), target_index_count, target_error, 0, &result_error));
	//lod.indices.resize(meshopt_simplifySloppy(&lod.indices[0], &mesh.indices[0], mesh.indices.size(), &mesh.vertices[0].px, mesh.vertices.size(), sizeof(Vertex), target_index_count, target_error, &result_error));
  double tsimp = timestamp();
  std::cout << "simp time " << (tsimp - t0) << "\n";
  
	lod.vertices.resize(lod.indices.size() < mesh.vertices.size() ? lod.indices.size() : mesh.vertices.size()); // note: this is just to reduce the cost of resize()
	lod.vertices.resize(meshopt_optimizeVertexFetch(&lod.vertices[0], &lod.indices[0], lod.indices.size(), &mesh.vertices[0], mesh.vertices.size(), sizeof(Vertex)));
	double t1 = timestamp();

	std::cout <<"remove unused vert time "<< (t1 - tsimp) << "\n";
	SaveObj(outFile, lod);
  double tsave = timestamp();
	std::cout << "save obj time " << (tsave-t1) << "\n";
}
...
int main(int argc, char** argv)
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		{
			i++;
			target_error = float(std::atof(argv[i]));
		}
		else if (a == "-c")
		{
			i++;
			target_count_frac = float(std::atof(argv[i]));
		}
		else
		{
			inFile = argv[i];
	...
	std::cout << "dir " << dir << "\\, file " << fname << "\n";
	std::string prefix = fname;
	size_t lastindex = fname.find_last_of(".");
	if (lastindex != fname.npos)
	{
		prefix = fname.substr(0, lastindex); 
	}
	outFile = dir+"/"+prefix + "_simp.obj";
	std::cout<<"out file "<<outFile<<"\n";
	TestSimplify(inFile, outFile, target_error, target_count_frac);
	/*void runTests();
