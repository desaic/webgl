...
export function Init(value: HTMLCanvasElement | WebGL2RenderingContext | WebGLRenderingContext | CanvasRenderingContext2D | null): void {
    ...
    io.SetClipboardTextFn = (_user_data: any, text: string): void => {
        clipboard_text = text;
...
export function RenderDrawData(draw_data: ImGui.DrawData | null = ImGui.GetDrawData()): void {
    ...
    if (gl2) {
        gl2.bindVertexArray(vertex_array_object)
    } else if (gl_vao) {
        gl_vao.bindVertexArrayOES(vertex_array_object);
    }
    ...
    if (gl2) { gl2.deleteVertexArray(vertex_array_object); } else if (gl_vao) { gl_vao.deleteVertexArrayOES(vertex_array_object); } 
    ...
    if (gl2) {
        gl2.bindVertexArray(last_vertex_array_object);
    } else if (gl_vao) {
        gl_vao.bindVertexArrayOES(last_vertex_array_object);
    } 
