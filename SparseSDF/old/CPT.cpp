...
/// voxelizes 1 triangle.
/// The triangle is inflated into an oriented bounding box (OBB)
/// where each axis contains a margin of sdf->exactBand.
void Voxelize(size_t tidx, SDFGridTree* sdf);

/// voxel center and triangle distance.
/// voxel centers are defined on integer coordinates
/// e.g. (0,0)
void ExactDistance(SDFGridTree* sdf);
void ExactDistanceBruteForce(SDFGridTree* sdf, std::vector<unsigned>& minTIdx);
...
void SDFGridTree::Allocate(Vec3u gridSize) {
  sdf.Allocate(gridSize[0], gridSize[1], gridSize[2]);
  idxGrid.Allocate(gridSize[0], gridSize[1], gridSize[2]);
}

int cpt(SDFGridTree& sdf)
{
  BBox box;
  ...
  float margin = h * (sdf.conf.band);

  for (unsigned i = 0; i < 3; i++) {
    gridSize[i] = (box.vmax[i] - box.vmin[i] + 2 * margin) / h;
    sdf.conf.gridOrigin[i] = box.vmin[i] - margin;
    if (gridSize[i] > MAX_GRID_SIZE) {
      std::cout << "grid size is too large. check config." << gridSize[i] << "\n";
      return -1;
    }
  }
  sdf.Allocate(gridSize);
...
unsigned closestTrig(const std::vector<size_t>& trigs,
  SDFGridTree * sdf, size_t x, size_t y, size_t z,
  TrigDistInfo& minInfo)
{
  float h = sdf->conf.voxelSize;
  Vec3f corner = sdf->conf.gridOrigin + h * Vec3f(x, y, z);
...
void AddTrigs(SDFGridTree* sdf, TreePointer& ptr, std::set<size_t>& s)
{
  if (!ptr.HasValue()) {
    return;
  }
...
void GetNeighborTrigs(std::vector<size_t>& trigs, SDFGridTree* sdf, size_t x,
                      size_t y, size_t z,
...
void AddTrigToVoxel(size_t tidx, TreePointer* ptr, SDFGridTree* sdf) {
  ptr->CreatePath();
  size_t listIdx = 0;
  if (ptr->HasValue()) {
    GetVoxelValue(*ptr, listIdx);
...
void Voxelize(size_t tidx, SDFGridTree* sdf) {
  ...
  std::vector<float> trig;
  GetTrig(trig, tidx, sdf->mesh, sdf->conf.gridOrigin);
  const unsigned dim = 3;
  ComputeOBB(trig, obb, sdf->conf.exactBand * sdf->conf.voxelSize);
  const Vec3u& gridSize = sdf->idxGrid.GetSize();
  float voxelSize = sdf->conf.voxelSize;
  ScaleOBB(1.0 / voxelSize, obb);

  //voxelize the oriented bounding box (OBB) of a triangle.
  SparseVoxel<int> voxels;
  OBBSlicer slicer;
  slicer.Compute(obb, voxels);
  TreePointer ptr(&sdf->idxGrid);

  //add this triangle to all voxels covered by this obb
  //as a potential closest triangle.
