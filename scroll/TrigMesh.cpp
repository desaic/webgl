...
float TrigMesh::GetArea(unsigned tIdx) const {
  ...
  Vec3f e10 = Vec3f(v[vIdx[1]*3], v[vIdx[1]*3 + 1], v[vIdx[1]*3 + 2]) - v0;
  Vec3f e20 = Vec3f(v[vIdx[2]*3], v[vIdx[2]*3 + 1], v[vIdx[2]*3 + 2]) - v0;
...
Vec3f TrigMesh::GetRandomSample(const unsigned tIdx, const float r1, const float r2) const {
  unsigned vIdx[3] = {t[tIdx*3], t[tIdx*3 + 1], t[tIdx*3 + 2]};
  Vec3f v0(v[vIdx[0]*3], v[vIdx[0]*3 + 1], v[vIdx[0]*3 + 2]);
  Vec3f v1(v[vIdx[1]*3], v[vIdx[1]*3 + 1], v[vIdx[1]*3 + 2]);
  Vec3f v2(v[vIdx[2]*3], v[vIdx[2]*3 + 1], v[vIdx[2]*3 + 2]);
  return (1.f - sqrtf(r1))*v0 + (sqrtf(r1)*(1.f - r2))*v1 + (r2*sqrtf(r1))*v2;
}

// also returns trig normal and a tangent vector
void TrigMesh::GetRandomSample(const unsigned tIdx, const float r1, const float r2, Vec3f& pos, Vec3f& normal, Vec3f& tangent) const {
 unsigned vIdx[3] = {t[tIdx*3], t[tIdx*3 + 1], t[tIdx*3 + 2]};
  Vec3f trigV[3] = {
  ...
  pos = (1.f - sqrtf(r1))*trigV[0] + (sqrtf(r1)*(1.f - r2))*trigV[1] + (r2*sqrtf(r1))*trigV[2];
...
void TrigMesh::SaveObj(const std::string& filename, const std::vector<Vec3f>& pts) {
  std::ofstream out(filename);
  if (! out.is_open())
    return;

  for (const auto& p : pts) {
    out << "v " << std::setprecision(12) << p[0] << " " << p[1] 
      << " " << p[2] << "\n";
  }
  out.close();
}
...
int TrigMesh::SaveStlTxt(const std::string& filename)
{
  ...
  size_t numVerts = v.size() / 3;
  size_t numTrig = t.size() / 3;
  if (nt.size() != numTrig * 3) {
    ComputeTrigNormals();
  }
  for (size_t tIdx = 0; tIdx < numTrig; tIdx++) {
    out << "facet normal ";
    out << nt[3 * tIdx] << " " << nt[3 * tIdx + 1] << " " << nt[3 * tIdx + 2]<<"\n";
    out<<"outer loop\n";
    for (unsigned vi = 0; vi < 3; vi++) {
      size_t vIdx = t[3 * tIdx + vi];
      out << "vertex ";
      out << v[3 * vIdx] << " " << v[3 * vIdx + 1] << " " << v[3 * vIdx + 2] << "\n";
    }
    out << "endloop\n";
    out << "endfacet\n";
  }
