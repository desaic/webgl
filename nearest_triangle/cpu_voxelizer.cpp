...
static Vec3f clamp(const Vec3f& v, const Vec3f& lb, const Vec3f& ub) {
  return Vec3f(std::clamp(v[0], lb[0], ub[0]), std::clamp(v[1], lb[1], ub[1]),
               std::clamp(v[2], lb[2], ub[2]));
}

// Mesh voxelization method
void cpu_voxelize_mesh_cb(VoxConf conf, const TrigMesh *mesh,
...
static bool InBound(int x, int y, int z, const Vec3u & size) {
  return x >= 0 && x < int(size[0]) && y >= 0 && y < int(size[1]) && z >= 0 && z < int(size[2]);
}
...
void cpu_voxelize_grid(VoxConf conf, const TrigMesh *mesh, Array3D8u &grid) {
  ...
  for (size_t i = 0; i < n_triangles; i++) {
    ...
    // divided by the grid unit vector
    IntBox t_bbox_grid;
    Vec3f gridvec = clamp(t_bbox_world.vmin / conf.unit, Vec3f(0.0f, 0.0f, 0.0f), grid_max);
    t_bbox_grid.min = Vec3i(gridvec[0], gridvec[1], gridvec[2]);
    gridvec = clamp(t_bbox_world.vmax / conf.unit, Vec3f(0.0f, 0.0f, 0.0f), grid_max);
    t_bbox_grid.max = Vec3i(gridvec[0], gridvec[1], gridvec[2]);
    ...
    float d_xy_e0 = (-1.0f * n_xy_e0.dot(Vec2f(v0[0], v0[1]))) + std::max(0.0f, delta_p[0] * n_xy_e0[0]) +
                    std::max(0.0f, delta_p[1] * n_xy_e0[1]);
    float d_xy_e1 = (-1.0f * n_xy_e1.dot(Vec2f(v1[0], v1[1]))) + std::max(0.0f, delta_p[0] * n_xy_e1[0]) +
                    std::max(0.0f, delta_p[1] * n_xy_e1[1]);
    float d_xy_e2 = (-1.0f * n_xy_e2.dot(Vec2f(v2[0], v2[1]))) + std::max(0.0f, delta_p[0] * n_xy_e2[0]) +
                    std::max(0.0f, delta_p[1] * n_xy_e2[1]);
    // YZ plane
    Vec2f n_yz_e0(-1.0f * e0[2], e0[1]);
    Vec2f n_yz_e1(-1.0f * e1[2], e1[1]);
    ...
    float d_yz_e0 = (-1.0f * n_yz_e0.dot(Vec2f(v0[1], v0[2]))) + std::max(0.0f, delta_p[1] * n_yz_e0[0]) +
                    std::max(0.0f, delta_p[2] * n_yz_e0[1]);
    float d_yz_e1 = (-1.0f * n_yz_e1.dot(Vec2f(v1[1], v1[2]))) + std::max(0.0f, delta_p[1] * n_yz_e1[0]) +
                    std::max(0.0f, delta_p[2] * n_yz_e1[1]);
    float d_yz_e2 = (-1.0f * n_yz_e2.dot(Vec2f(v2[1], v2[2]))) + std::max(0.0f, delta_p[1] * n_yz_e2[0]) +
                    std::max(0.0f, delta_p[2] * n_yz_e2[1]);
    // ZX plane
    Vec2f n_zx_e0(-1.0f * e0[0], e0[2]);
    Vec2f n_zx_e1(-1.0f * e1[0], e1[2]);
    ...
    float d_xz_e0 = (-1.0f * n_zx_e0.dot(Vec2f(v0[2], v0[0]))) + std::max(0.0f, delta_p[0] * n_zx_e0[0]) +
                    std::max(0.0f, delta_p[2] * n_zx_e0[1]);
    float d_xz_e1 = (-1.0f * n_zx_e1.dot(Vec2f(v1[2], v1[0]))) + std::max(0.0f, delta_p[0] * n_zx_e1[0]) +
                    std::max(0.0f, delta_p[2] * n_zx_e1[1]);
    float d_xz_e2 = (-1.0f * n_zx_e2.dot(Vec2f(v2[2], v2[0]))) + std::max(0.0f, delta_p[0] * n_zx_e2[0]) +
                    std::max(0.0f, delta_p[2] * n_zx_e2[1]);
