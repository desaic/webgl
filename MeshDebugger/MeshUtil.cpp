...
void addLayer(QuadMesh* mpt, int layer0, int layer1, int faceIdx, const Array3D8u* volpt, int mat,
              const Vec3f* dxpt) {
  QuadMesh& m = (*mpt);
...
void addFaces(QuadMesh& m, int faceIdx, const Array3D8u& vol, int mat, const Vec3f& dx) {
  int axis = faceIdx / 2;
...
void save_obj(const QuadMesh& m, const std::string& filename) {
  ...
  for (size_t ii = 0; ii < m.q.size(); ii++) {
    out << fTok << " " << m.q[ii][0] + 1 << " " << m.q[ii][1] + 1 << " " << m.q[ii][2] + 1 << " "
        << m.q[ii][3] + 1 << "\n";
  }
  out << "#end\n";
  out.close();
...
uint8_t LinearIndex(int x, int y, int z, std::vector<uint8_t>& vol,
                    std::array<unsigned, 3>& gsize) {
  return vol[gsize[0] * (gsize[1] * z + y) + x];
}

void MarchOneCube(int x, int y, int z, TrigMesh& m, std::vector<uint8_t>& vol,
                  std::array<unsigned, 3>& gsize) {
  GridCell cell;
