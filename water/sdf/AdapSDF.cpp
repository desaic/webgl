...
class CoarseTrigTask : public TPTask {
 public:
  ...
  CoarseTrigTask(AdapSDF* sdf, unsigned z0, unsigned z1) : _sdf(sdf), _z0(z0), _z1(z1) {}
...
class PtTrigTask : public TPTask {
 public:
  ...
  PtTrigTask(AdapSDF* sdf, unsigned z0, unsigned z1) : _sdf(sdf), _z0(z0), _z1(z1) {}
  virtual ~PtTrigTask() {}
...
bool OutsideNarrowBand(const Array3D<short>& dist, unsigned ix, unsigned iy, unsigned iz) {
  short corners[8];
  ...
  for (int i = 0; i < 8; i++) {
    if (corners[i] >= AdapSDF::MAX_DIST - 1) {
      return true;
    }
  }
  return false;
...
int SDFImpAdap::Compute() {   
  ...
  Array3D8u frozen;
  timer.Start();
  FastSweepPar(sdf.dist, sdf.voxSize, sdf.distUnit, sdf.band, frozen);

  Vec3u distSize = sdf.dist.GetSize();
  Array2D8u slice(distSize[0],distSize[1]);
  unsigned z= distSize[2] / 2;
  for (unsigned row = 0; row < distSize[1]; row++) {
    for (unsigned col = 0; col < distSize[0]; col++) {
      float d = (sdf.dist(col, row, z) * sdf.distUnit + 2);
      d = d * 50;
      if (d < 0) {
        d = 0;
      }
      ...
      slice(col, row) = uint8_t(d);
    }
  }

  SavePngGrey("dist_slice.png",slice);

  ms = timer.ElapsedMS();
  sdf.ComputeFineDistBrute();
  sdf.ClearTemporaryArrays();
  return 0; 
}
...
float SDFImpAdap::DistTrilinear(Vec3f coord) { 
  float dist = sdf.GetFineDist(coord);
  return dist;
}
  ...
  for (unsigned x = 0; x < s[0] - 1; x++) {
    for (unsigned y = 0; y < s[1] - 1; y++) {
      for (unsigned z = 0; z < s[2] - 1; z++) {
        MarchOneCube(x, y, z, grid, level / sdf.distUnit, surf, sdf.voxSize / sdf.distUnit);
      }
    }
  }
  for (unsigned i = 0; i < surf->v.size(); i += 3) {
    surf->v[i] = sdf.distUnit * surf->v[i] + sdf.origin[0];
    surf->v[i+1] = sdf.distUnit * surf->v[i+1] + sdf.origin[1];
    surf->v[i+2] = sdf.distUnit * surf->v[i+2] + sdf.origin[2];
  }
}
